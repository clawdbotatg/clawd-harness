...
# (The 2026-08-30 skills sync — library → ~/.claude/skills on every box — was
# REMOVED the same day: the library is deliberately decoupled from machines.
# Skills are pasted into sessions by the 📚 picker, straight from the relay
# store; nothing installs anywhere. One-shot below cleans up what it wrote.)


def _skills_sync_cleanup(home=None):
    """Remove what the short-lived sync installed: the managed skills named in
    ~/.clawd-fleet.skills.json (files it tracked, nothing else) + the state
    file itself. No-op when the state file is absent (i.e. everywhere, soon)."""
    home = home or Path.home()
    state_file = home / ".clawd-fleet.skills.json"
    try:
        ...
        for root in roots:
            ...
            if d.is_dir():
                ...
                try:
                    ...
                    pass   # user files inside — leave it
    try:
        state_file.unlink(missing_ok=True)
        print(f"{ts()} [skills] cleaned up the retired skills sync "
              f"({', '.join(state)})", flush=True)
    except OSError:
        pass
...
class Worker:
    ...
    def run(self):
        ...
        if self.kind != "relay":
            ...
            threading.Thread(target=_skills_sync_cleanup, daemon=True).start()
        if SELF_RESTART:
