...
{ "type":"closed", "closed":[{cid,engine,pid,project,session_id,title,desc,account,prompt_count,first_prompt,created,last_active,closed_at,reason,resumable}] }   // 🗃️ closed-session history, newest first (every `close`, capped at CLOSED_MAX, persisted); on `list` and on every change. `reason`: "closed" | "project gone". `resumable`=false = no conversation to resume (reopen starts fresh).
