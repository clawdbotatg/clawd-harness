...
| `fork` | `cid` | ⑂ Fork a LIVE session: a new session in the same project continuing `cid`'s conversation under its OWN engine id (`claude --resume <id> --fork-session`; the source keeps running, untouched). Replies `{type:"focus", cid}` like `new`. Refused — `{type:"error", cid, fork: cid, error}` (`fork` names the source so a client can land back on it) — for codex sessions, sign-in ceremonies, a session with no conversation yet, or an unknown cid. |
| `reopen` | `cid` | 🗃️ Bring back a CLOSED session by its history row (`closed` frame): a new session in the same project running the engine's resume of the old conversation (`create_session(resume=…)`). Replies `{type:"focus", cid}` like `new`, and re-broadcasts `closed` without the row. If the row's project is gone: `{type:"error", cid, error}` and the row stays. |
| `closedForget` | `cid?` | 🗃️ Drop one history row (or every row when `cid` is omitted). Broadcasts `closed`. |
| `wrap` | `cid`, `text?`, `via?` | 📑 Arm the session to close ITSELF and deliver the wrap prompt (`text`, else the server's `WRAP_PROMPT`; a literal `{file}` in either becomes this wrap's `HANDOFF-<stamp>.md`: write the handoff to that LOCAL file at the repo root — one new file per wrap, older ones stay readable — the arm lists `HANDOFF-*.md` in the checkout's `.git/info/exclude`, it is never committed or pushed — commit/push any real work, run `harness-close`, end with a TLDR). The arm lasts `WRAP_TURNS` (2) Stops / `WRAP_TTL_S` (30 min); a sign-in or autopilot session can't be armed → `{type:"error", cid, error:"wrap: …"}`. The session's `harness-close` (bin/, URL in its `HARNESS_CLOSE_URL`) POSTs `/self/close?t&cid` — refused unless armed (403), or on a ceremony/autopilot session (409); a dirty git worktree no longer refuses (2026-09-11) — the close is accepted and the porcelain lines are appended to the reply as a note (untracked root `HANDOFF-*.md` files — and a pre-09-15 `HANDOFF.md` — are never listed); accepted (200) = `wrapClosing`, the close happens at the next Stop (grace 20 s) with history `reason:"wrapped"`. The harness never force-closes: no call → the arm lapses, the tab stays. |
| `wrapCancel` | `cid` | 📑 Disarm (and defuse an accepted close). Broadcasts `sessions`. |
| `check` | `cid`, `via?` | 🔍 Double-check: the OTHER engine reviews this session's work (codex for a claude session, claude for a codex one). The server arms the session (`checkArmed`, volatile, `CHECK_TTL_S` = 30 min), lists `REVIEW-*.md` in the checkout's `.git/info/exclude` (never committed, like `HANDOFF-*.md`; one `REVIEW-<stamp>.md` per review), and types the brief prompt (`CHECK_BRIEF_PROMPT`: what it did, where, what it verified, what it's unsure of, what's uncommitted — one turn, no code changes). The first Stop AFTER that prompt was submitted (a tap mid-turn waits; the arm remembers `promptCount`) spawns a reviewer session in the same project via `new`'s path with `check_of` = this cid, waits for its TUI (`CHECK_READY_S`), and types `CHECK_REVIEW_PROMPT`: read the brief as CLAIMS, the truth is `git diff <head_at_spawn>..HEAD` (every session records its HEAD at spawn; a pre-feature row falls back to commits since `created`) + the working tree, run the repo's tests, never edit, end with a severity-tagged issue list + a 3-line TLDR. The reviewer's first Stop closes the loop (`_check_back_on_stop` → `check_back`): the harness appends the reviewer's final message to the review file under `## Review by <engine>` and types `CHECK_ACT_PROMPT` into the SOURCE — read the findings, think critically, fix what's right, say why not for the rest, don't commit the file. The reviewer row's `check_file`/`check_pending` are registry fields so a restart in between still hands back; its `desc` then reads "reviewed → findings sent back to …". No direct reply on success — the source's `sessions` row shows `checkArmed`, the reviewer arrives in a later `sessions` frame with `checkOf`. Refused → `{type:"error", cid, check: cid, error:"can't double-check: …"}` (`check` names the source so a client can stay put) for a sign-in ceremony, a session with no conversation yet, codex not signed in, or an unknown cid. The source keeps running untouched. |
| `checkCancel` | `cid` | 🔍 Disarm a pending double-check (the brief still gets written; no reviewer spawns). Broadcasts `sessions`. |
...
| `tldr` | `cid`, `on?`, `mark?`, `voice?` | 🔊 `voice:true/false` = this viewer has (or no longer has) both 🟦 and 🔊: the server reads the summary aloud for the session — each sentence once it is settled (unchanged from the previous pass), the rest at the final pass, a reply too short for a summary as is, a permission/question notification at once — as `say` frames (below). No separate model decides what to say. 🟦 `mark:true` = the viewer dismissed the summary: read this far — the server forgets the current summary and later passes cover only prose streamed after this point (blank `tldr` frame goes out). Otherwise: this viewer wants (`on:true`, the default) or no longer wants the **live TLDR** of a session: a rolling plain-English summary of the turn in flight, streamed as `tldr` frames to that session's subscribers. Volatile per viewer but the flags persist per session (registry) — a browser re-sends them right after every `subscribe`. Turning it on mid-turn catches up on the text streamed so far; after a turn, one final pass over the last reply. Source is the API tee (`server.py` `API_TEE`: the session's `ANTHROPIC_BASE_URL` points at a local pass-through proxy that copies assistant text deltas as they stream); summarizer is one `claude -p --model haiku` (`TLDR_MODEL`) at a time under the session's account. Claude engine only. |
| `tts` | `id`, `text` | 🔊 ElevenLabs over the socket (the fleet path: the relay doesn't proxy `/tts` and a browser can't reach a machine's HTTP; the text rides the E2E channel like any frame). Replies to this client only: `{type:"tts", id, audio:<base64 mp3>}` or `{type:"tts", id, error}` (no key on the box / upstream failure) — the browser then uses its own voice for that line. Direct mode keeps the streaming `POST /tts`. |
| `stt` | `id` | 🎤 Deepgram dictation credential for the page's OWN socket to `wss://api.deepgram.com/v1/listen` (the mic streams straight there — never through the relay — so the browser needs a credential in hand). Replies to this client only: `{type:"stt", id, proto:"bearer"|"token", secret, ttl, model}` — `bearer` = a temporary JWT the box minted (`POST /v1/auth/grant`, needs a Member-level key; cached, re-minted a minute before expiry; `ttl` seconds left), `token` = the API key itself when the key can't mint (`ttl` 0). Or `{type:"stt", id, error:"stt not configured"}` (no `DEEPGRAM_API_KEY` on the box) — the browser dictates with its own Web Speech recognizer. The page asks on every `subscribe` (creds are per box) and decides the engine AT THE PRESS from creds already in hand; the browser passes `[proto, secret]` as the WebSocket subprotocol and biases nova-3 with `keyterm`s (the ⚙️ word list, the harness's own names, every project name). |
...
| `send` | `text`, `cid?`, `via?` | High-level: type `text`, wait for the paste to settle, then submit `\r`. Use this to "send a message/prompt". Optional `via` tags the send's origin (`"quick"` = a quick-prompt chip tap) for the server's prompt log; omitted = typed. Optional `id`: the server answers the sender `{type:"sendAck", id, cid}` the moment the frame arrives (before delivery into the CLI), or `{type:"sendAck", id, error:"no session"}` when the session is unknown — the page's "✓ on <machine>" / "not delivered" marker. |
...
{ "type":"closed", "closed":[{cid,engine,pid,project,session_id,title,desc,account,prompt_count,first_prompt,created,last_active,closed_at,reason,last_answer,resumable}] }   // 🗃️ closed-session history, newest first (every `close`, capped at CLOSED_MAX, persisted); on `list` and on every change. `reason`: "closed" | "project gone" | "wrapped" (📑 closed itself; `last_answer` = its TLDR, ≤280 chars). `resumable`=false = no conversation to resume (reopen starts fresh).
...
{ "type":"say", "cid", "text", "urgent":bool, "turn":int }   // 🔊 the voice: one summary sentence to read aloud, sent as it settles
                                                       // (urgent = a permission/question notification, spoken at once)
{ "type":"tldr", "cid", "text", "final":bool, "turn":int,
  "sents":[[<sentence>, <settled:bool>], …] }           // 🟦 to that session's subscribers (see the `tldr` verb); `sents` = the
                                                       // annealing: a sentence that came back unchanged from the previous pass is settled
...
{ "type":"sendAck", "id", "cid?", "error?" }           // to the sender of a `send` that carried an id:
                                                       // the machine has it (error = no such session)
```
...
{ "cid", "pid", "title", "desc", "tab", "named":bool, "busy":bool, "waiting":bool,
  ...
  "wrapArmed":bool, "wrapClosing":bool, "checkArmed":bool, "checkOf":str }
  // 📑 wrapArmed = may close itself (the human's wrap chip / the PM's wrap verb);
  // wrapClosing = harness-close accepted, closes at the next Stop
  // 🔍 checkArmed = writing its brief; a reviewer spawns at its next Stop (see `check`);
  // checkOf = on a reviewer session, the cid of the session it double-checks ("" otherwise)
...
- `loginCta` = `{name, org, reason, pct}` or `null` — the 🔑 **"sign in to X"**
  ...
  sends `accountAdd` for that name (the same re-sign-in path as the 🧠 card).
