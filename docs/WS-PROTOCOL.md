...
| `wrap` | `cid`, `text?`, `via?` | 📑 Arm the session to close ITSELF and deliver the wrap prompt (`text`, else the server's `WRAP_PROMPT`: write the handoff, commit/push, run `harness-close`, end with a TLDR). The arm lasts `WRAP_TURNS` (2) Stops / `WRAP_TTL_S` (30 min); a sign-in or autopilot session can't be armed → `{type:"error", cid, error:"wrap: …"}`. The session's `harness-close` (bin/, URL in its `HARNESS_CLOSE_URL`) POSTs `/self/close?t&cid` — refused unless armed (403), or on a ceremony/autopilot session or a dirty git worktree (409, the porcelain lines in the body); accepted (200) = `wrapClosing`, the close happens at the next Stop (grace 20 s) with history `reason:"wrapped"`. The harness never force-closes: no call → the arm lapses, the tab stays. |
