...
| `wrap` | `cid`, `text?`, `via?` | 📑 Arm the session to close ITSELF and deliver the wrap prompt (`text`, else the server's `WRAP_PROMPT`: write the handoff to a LOCAL `HANDOFF.md` — the arm lists it in the checkout's `.git/info/exclude`, it is never committed or pushed — commit/push any real work, run `harness-close`, end with a TLDR). The arm lasts `WRAP_TURNS` (2) Stops / `WRAP_TTL_S` (30 min); a sign-in or autopilot session can't be armed → `{type:"error", cid, error:"wrap: …"}`. The session's `harness-close` (bin/, URL in its `HARNESS_CLOSE_URL`) POSTs `/self/close?t&cid` — refused unless armed (403), or on a ceremony/autopilot session (409); a dirty git worktree no longer refuses (2026-09-11) — the close is accepted and the porcelain lines are appended to the reply as a note (an untracked root `HANDOFF.md` is never listed); accepted (200) = `wrapClosing`, the close happens at the next Stop (grace 20 s) with history `reason:"wrapped"`. The harness never force-closes: no call → the arm lapses, the tab stays. |
