...
TOOLS = [
    ...
    ("check", "🔍 Double-check a session's work with the OTHER engine (codex "
        ...
        "brief (REVIEW-<stamp>.md, one per review, never committed); when that turn "
        "ends the harness spawns a reviewer session in the same project that treats "
        "the brief as claims and the diff since the source started as truth, runs "
        "the repo's tests, never edits, and ends with a severity-tagged issue list + "
        "TLDR. When the reviewer finishes, the harness appends its verdict to the "
        "review file and prompts the SOURCE to act on it (think critically, fix "
        "what's right, say why not for the rest) — you do not have to relay it. The "
        "reviewer shows in the roster with checkOf = the source cid; its lastAnswer "
        "is the verdict, the source's next lastAnswer is what it did about it. "
        "Refused for a sign-in session, no conversation yet, or codex not signed "
        "in. WRITE.",
        _S({"machine": _STR, "cid": _STR, "confirm": _BOOL}, ["machine", "cid"])),
